...
function requestRoute(req) {
  ...
  const apiPath = pathname.replace(/^\/api\/parents/, '') || '/'
...
async function authenticateInternalCaller(req, env) {
  ...
  if (!url || !anon || !service) {
    const error = new Error(
      'Parent accounts are not configured. Add SUPABASE_SERVICE_ROLE_KEY to the server .env (never prefix it with VITE_).',
    )
    error.status = 500
    throw error
  }
  ...
  if (!token) {
    const error = new Error('Please sign in again to manage parents.')
    error.status = 401
    throw error
  }
  ...
  if (!isInternalRole(normalizeRole(callerProfile.role))) {
    const error = new Error('You do not have permission to manage parents.')
    error.status = 403
    throw error
  }
...
async function handleCreateParent(req, res, env) {
  if (req.method !== 'POST') {
    json(res, 405, { error: 'Method not allowed.' })
    return
  }
  ...
  try {
    ...
    if (createError || !created.user) {
      const message = createError?.message || 'Unable to create the parent login.'
    ...
    const { data: parentRow, error: parentError } = await admin
      .from('parents')
      .upsert({ id: parentUserId }, { onConflict: 'id' })
      .select('id, created_at, updated_at')
      .single()

    if (parentError || !parentRow) {
      await admin.auth.admin.deleteUser(parentUserId)
      json(res, 500, { error: parentError?.message || 'Unable to save the parent record.' })
      return
    }
    ...
    if (readError || !profile) {
      json(res, 500, { error: readError?.message || 'Parent created but profile could not be loaded.' })
      return
    }
    ...
    json(res, error.status || 500, {
      error: error instanceof Error ? error.message : 'Unable to add this parent.',
    })
  }
}

export function createParentsMiddleware(env) {
  return async (req, res) => {
    ...
    if (route === '/' || route === '') {
      await handleCreateParent(req, res, env)
      return
    }
