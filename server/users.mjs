...
function requestRoute(req) {
  ...
  const apiPath = pathname.replace(/^\/api\/users/, '') || '/'
  return { url, route: apiPath }
}

async function authenticateUserAdmin(req, env) {
  const url = supabaseUrl(env)
  const anon = publishableKey(env)
  const service = serviceRoleKey(env)
  if (!url || !anon || !service) {
    const error = new Error(
      'User accounts are not configured. Add SUPABASE_SERVICE_ROLE_KEY to the server .env (never prefix it with VITE_).',
    )
    error.status = 500
    throw error
  }
  ...
  if (!token) {
    const error = new Error('Please sign in again to manage users.')
    error.status = 401
    throw error
  }
...
async function handleCheckEmail(req, res, env, query) {
  ...
  try {
    const { admin } = await authenticateUserAdmin(req, env)
...
async function handleCreateUser(req, res, env) {
  if (req.method !== 'POST') {
    json(res, 405, { error: 'Method not allowed.' })
    return
  }
  ...
  try {
    const { admin, callerRole } = await authenticateUserAdmin(req, env)
    ...
    if (createError || !created.user) {
      const message = createError?.message || 'Unable to create this login.'
    ...
    try {
      await syncRoleEntityRecord(admin, role, userId)
    } catch (syncError) {
      await admin.auth.admin.deleteUser(userId)
      json(res, syncError.status || 500, { error: syncError.message })
      return
    }
    ...
    if (readError || !profile) {
      json(res, 500, { error: readError?.message || 'User created but profile could not be loaded.' })
      return
    }
    ...
    json(res, error.status || 500, {
      error: error instanceof Error ? error.message : 'Unable to create this user.',
    })
  }
}

export function createUsersMiddleware(env) {
  return async (req, res) => {
    ...
    if (route === '/' || route === '') {
      await handleCreateUser(req, res, env)
      return
    }
