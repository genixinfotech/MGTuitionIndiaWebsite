...
import { visibleOneViewNav } from '@/lib/oneview-nav'
import { useAuth } from '@/context/AuthContext'
import { cn } from '@/lib/utils'
import { useOneViewLayout } from '@/components/oneview/OneViewContext'

export function OneViewSidebar() {
  const { user } = useAuth()
  const { collapsed, mobileOpen, setMobileOpen, toggleCollapsed } = useOneViewLayout()
  const location = useLocation()
  const items = visibleOneViewNav(user?.role)
        ...
        >
          ...
          {!collapsed ? (
            <div className="min-w-0">
              <p className="truncate text-sm font-bold tracking-tight">OneView</p>
