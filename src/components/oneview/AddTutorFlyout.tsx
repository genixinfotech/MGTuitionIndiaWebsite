...
import { Eye, EyeOff, Loader2, Lock, Phone, Plus, Users, X } from 'lucide-react'
...
import { useTutorEmailCheck } from '@/hooks/useTutorEmailCheck'
import { createTutor } from '@/lib/tutors'
...
export function AddTutorFlyout({
  open,
  onClose,
  onCreated,
}: {
  open: boolean
  onClose: () => void
  onCreated: (tutor: Profile) => void
}) {
  const [form, setForm] = useState(emptyForm)
  const [saving, setSaving] = useState(false)
  const [error, setError] = useState('')
  const [showPassword, setShowPassword] = useState(false)
  const emailCheck = useTutorEmailCheck(form.email)
  ...
  async function handleSubmit(event: FormEvent) {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Unable to add this tutor.')
    } finally {
      setSaving(false)
    }
  }
      ...
      {open ? (
        ...
        >
          <button
            ...
            aria-label="Close add tutor panel"
            disabled={saving}
            onClick={onClose}
          />
          <motion.aside
            role="dialog"
            aria-modal="true"
            aria-labelledby="oneview-add-tutor-title"
          ...
          >
            <div className="flex items-start justify-between gap-4 border-b border-charcoal/[0.06] px-5 py-5">
              <div>
                <div className="flex items-center gap-2 font-bold text-charcoal">
                  ...
                  <h2 id="oneview-add-tutor-title">Add tutor</h2>
                </div>
                <p className="mt-1 text-sm text-charcoal/50">
                  Creates a tutor login, profile, and tutor record on the platform.
                </p>
              </div>
              <button
                type="button"
            ...
            <form onSubmit={(event) => void handleSubmit(event)} className="flex min-h-0 flex-1 flex-col">
              <div className="flex flex-1 flex-col gap-4 overflow-y-auto px-5 py-5">
                <FormField label="Full name" icon={Users}>
                  <input
                    required
              ...
              <div className="border-t border-charcoal/[0.06] px-5 py-4">
                ...
                >
                  {saving ? (
                    <>
                      <Loader2 className="h-4 w-4 animate-spin" /> Adding tutor…
                    </>
                  ) : (
                    <>
