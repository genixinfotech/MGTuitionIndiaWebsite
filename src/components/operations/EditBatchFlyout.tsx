...
import { listBatchNames, updateBatch, type Batch } from '@/lib/batches'
import { listQualityManagers, type StaffOption } from '@/lib/staff'
import { classTimings, classTimingIndex } from '@/lib/tuition-plans'
...
function formFromBatch(batch: Batch): BatchForm {
  const heroName = heroNameFromBatch(batch)
  const hero = heroName ? heroByName(heroName) : null
  ...
  return {
    ...
    heroName,
    heroFullName: hero?.fullName ?? batch.hero_full_name,
    startDate: batch.start_date,
    daysOfWeek: [...batch.days_of_week],
    timingIndex: classTimingIndex(batch.start_time, batch.end_time),
    minStudents: batch.min_students,
    maxStudents: batch.max_students,
    qualityManagerId: batch.quality_manager_id,
    tutorId: batch.tutor_id,
    meetingLink: batch.meeting_link?.trim() ?? '',
    notes: batch.notes?.trim() ?? '',
  }
}

export function EditBatchFlyout({
  open,
  batch,
  allocatedCount,
  onClose,
  ...
  const selectedSlot = classTimings[form?.timingIndex ?? 0] ?? classTimings[0]
  ...
  useEffect(() => {
    ...
    void Promise.all([listBatchNames(), listQualityManagers(), listTutors()])
      .then(([names, managers, tutorRows]) => {
        if (cancelled) return
        setUsedNames(names.filter((name) => name.trim().toLowerCase() !== batch.name.trim().toLowerCase()))
        setQualityManagers(managers)
        setTutors(tutorRows)
      })
      .catch((err) => {
        if (!cancelled) {
          setError(err instanceof Error ? err.message : 'Unable to load batch options.')
        }
      })
      .finally(() => {
  ...
  async function handleSubmit(event: FormEvent) {
    ...
    if (form.maxStudents < allocatedCount) {
      setError(`Maximum students cannot be below the ${allocatedCount} students already allocated.`)
      return
    }
    if (!previewBatchName) {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Unable to update this batch.')
    } finally {
      setSaving(false)
    }
  }
      ...
      {open && batch && form ? (
        ...
        >
          <button
            ...
            aria-label="Close edit batch panel"
            disabled={saving}
            onClick={onClose}
          />
          <motion.aside
          ...
          >
            <div className="flex items-start justify-between gap-4 border-b border-charcoal/[0.06] px-5 py-5">
              <div>
                ...
                <p className="mt-1 text-sm text-charcoal/50">{batch.name}</p>
              </div>
              <button
                type="button"
                onClick={onClose}
              ...
              <div className="min-h-0 flex-1 space-y-4 overflow-y-auto px-5 py-5">
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <FormField label="Max students">
                    <input
                      required
                      type="number"
                      min={Math.max(form.minStudents, allocatedCount)}
                      className={fieldClass()}
                      ...
                      onChange={(event) =>
                        patchForm({
                          maxStudents: Math.max(
                            form.minStudents,
                            allocatedCount,
                            Number(event.target.value) || form.minStudents,
                          ),
                        })
                      }
                    />
                ...
                {allocatedCount > 0 ? (
                  <p className="text-xs text-charcoal/45">
                    {allocatedCount} student{allocatedCount === 1 ? '' : 's'} allocated — max cannot go below this.
                  </p>
                ) : null}
